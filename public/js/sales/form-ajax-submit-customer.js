...
$(".ajax-submit-customer").submit(function(e){
    ...
    request.done(function(res, status, jqXhr) {
        ...
        generateToast("Successfully created customer.", 'bg-success');

        elm_customer_tagify.whitelist = [
            {
                "value": res.id,
                "name": res.name,
            }
        ...
        elm_customer_tagify.addTags(res.id);
        $(`tags.sp_customer`).find('span').html(elm_customer_tagify.whitelist[0].name);
        console.log(res.id);
    });
