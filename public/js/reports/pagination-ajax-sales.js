class PaginationAjaxSales {
    /**
     * Creates an instance of PaginationAjaxSales. 
     * 
    ...
    displayTableRows(rows) 
    {
        ...
        rows.forEach(function(row){           
            ...
            for(i = 0; i < columns.length; i++)
            {
                ...
                {
                    inner += `
                        <td>${row[columns[i]] == undefined || row[columns[i]] == 'null' 
                            ? 'Anonymous'
                            : row[columns[i]]
                        }</td>
                    `
                ...
                {
                    inner += `<td class="text-end">Php ${parseFloat(row[columns[i]]).toFixed(2)}</td>`;
                }
                else
                {
                    inner += `
        ...
        actionsAddSalesEventListeners(this.modelName, this.ajaxUrl, this.actions);
    }
...
function processSalesRequest(request, table)
{
    ...
    request.done(function(res, status, jqXHR) {
        ...
        $(`.btn-paginate-${table.modelName}`).click(function(e) {
            ...
            processSalesRequest(request, table);
        });
    });
...
function displaySalesActionButtons(id, modelName, actions, customActionsHtml = undefined)
{
    var inner = '';
...
function actionsAddSalesEventListeners(modelName, ajaxUrl, actions)
{
