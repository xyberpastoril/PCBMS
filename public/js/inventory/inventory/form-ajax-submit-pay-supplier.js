...
$("#form-return-expired-products").submit(function(e){
    console.log(`Submitting form ${e.target.id}`);
    console.log(e);
    e.preventDefault();

    closeButtonElement = $(`#close-form-return-expired-products`);
    submitButtonElement = $(`#submit-form-return-expired-products`);
    ...
    request.done(function(res, status, jqXhr) {
        ...
        generateToast('Successfully returned expired products', 'bg-success');
...
function showFormErrorsReceiveProducts(errors)
{
    ...
    error_list.forEach(function(e){
        ...
        if(Array.isArray(err)) {
            if(err.length == 2) {
                let errElm = $(`#form-return-expired-products .error-rep_${err[0]}`);
