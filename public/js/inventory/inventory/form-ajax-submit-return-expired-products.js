...
$("#form-pay-supplier").submit(function(e){
    console.log(`Submitting form ${e.target.id}`);
    console.log(e);
    e.preventDefault();

    closeButtonElement = $(`#close-form-pay-supplier`);
    submitButtonElement = $(`#submit-form-pay-supplier`);
    ...
    request.done(function(res, status, jqXhr) {
        ...
        generateToast('Successfully paid suppliers.', 'bg-success');
...
function showFormErrorsReceiveProducts(errors)
{
    ...
    error_list.forEach(function(e){
        ...
        if(Array.isArray(err)) {
            if(err.length == 2) {
                let errElm = $(`#form-pay-supplier .error-ps_${err[0]}`);
