var elm_tagify_unit_edit;

class ProductsPaginationAjax {
    /**
    ...
    displayTableRows(rows) 
    {
        ...
        rows.forEach(function(row){           
            ...
            <tr>
                <td>
                    ${displayProductActionButtons(row.uuid, modelName, actions)}
                </td>
                `
        ...
        productActionsAddEventListeners(this.modelName, this.ajaxUrl, this.actions);
    }
...
function processProductRequest(request, table)
{
    ...
    request.done(function(res, status, jqXHR) {
        ...
        $(`.btn-paginate-${table.modelName}`).click(function(e) {
            ...
            processProductRequest(request, table);
        });
    });
...
function displayProductActionButtons(id, modelName, actions, customActionsHtml = undefined)
{
    var inner = '';
...
function productActionsAddEventListeners(modelName, ajaxUrl, actions)
{
            ...
            $(`.btn-edit-${modelName}`).click(function(e) {
                ...
                request.done(function(res, status, jqXHR){
                    ...
                    inputElements.forEach(function(i){
                        console.log(i);
                        if(i.dataset.tagify == "true")  
                        {
                            console.log("Testing tagify.");
                            var request2 = $.ajax({
                                url: `/ajax/${i.name}s/id/${res[`${i.name}_id`]}`,
                                headers: { 'X-Requested-With': 'XMLHttpRequest' },
                                method: 'GET',
                            });

                            request2.done(function(res, status, jqXHR) {
                                console.log("Testing tagify success.");
                                elm = document.querySelector(`#input-form-edit-${modelName}-${i.name}`);
                                elm_tagify_unit_edit = window[`initTagify${capitalizeFirstLetter(i.name)}`](elm, res);
                            });
                        ...
                        else if(i.name != '_token' && i.name != '_method')
                        {
